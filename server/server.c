...
int handle_port_command(char* request_content, int client_fd){
	...
	*/
	if(bind(data_sock, (struct sockaddr *)&data_server_addr, sizeof(data_server_addr)) < 0){
        	send(client_fd, "425 Can't open data connection\r", strlen("425 Can't open data connection\r\n"), 0);
        	perror("Bind failed!");
		return -1;
	}
	...
	*/
	printf("\nConnecting to File Transfer Socket\n");
	...
	*/
    	switch (req) {
			...
			*/
			...
			int result = handle_port_command(buffer, client_fd);
		...
		case 3: // STOR
			...
			if (fork()==0){
				int result = handle_stor_command(buffer, client_fd);
				if (result==1){
		...
		case 4: // RETR
		    	...
		    	if (fork() == 0){
				int result = handle_retr_command(buffer, client_fd);
				if (result == 1){
					send(client_fd,"226 Transfer completed.\r\n", strlen("226 Transfer completed.\r\n"), 0);
			}
		        	exit(0);
